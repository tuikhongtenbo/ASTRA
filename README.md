...
[Ảnh gốc + Câu hỏi SpatialMQA]
       ...
       │
       ├─► [Module 1: OGM] ── Grounding DINO-Tiny ──► Phát hiện bbox O1, O2 & Vẽ Set-of-Mark [1], [2] lên ảnh
       │
...
* **Module 1 — Object-Grounded Marking (OGM):**
  ...
  * Sử dụng model nhúng nhẹ **Grounding DINO-Tiny (~172M params)** để phát hiện bounding box.
...
# 1b. Cài đặt openai (cho scripts/extract_objects.py và LLM extractor)
pip install openai

# 2. Cài đặt Grounding DINO (cho Module 1 OGM)
...
> [!NOTE]
> Các model bổ trợ (**Grounding DINO-Tiny** và **Depth-Anything-V2-Small**) có kích thước cực kỳ nhỏ nhẹ (tổng cộng < 200M params), gần như không tiêu tốn đáng kể VRAM GPU (< 1 GB) khi chạy song song cùng Qwen3-VL.
...
│   ├── __init__.py
│   ├── module1_ogm.py       # Module 1: Grounding DINO Set-of-Mark bounding boxes
...
**Yêu cầu:** Set biến môi trường `QWEN_API_KEY` hoặc tạo file `.env` trong thư mục gốc.
